...
class OrderController extends Controller
{
    ...
    public function updateOrderDetail(Request $request)
    {
        ...
        if($order->status !== 'PENDING') {
            return response()->json([
                'message' => 'Cannot update order details for a order',
            ]);
        }

        $orderDetail = OrderDetail::find([$validator['order_id'], $validator['product_id']]);
        $orderDetail->quantity = $validator['quantity'];
        $orderDetail->save();
     ...
     */
     public function deleteOrderDetail(Request $request){
        ...
        $orderDetail = OrderDetail::find([$request->order_id ,$request->product_id]);
